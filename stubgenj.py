...
def is_internal(name: str) -> bool:
    ...
    if re.match(".*\\$\\d+", name):
        return True  # anonymous inner classes
    if name.endswith('$SerialForm'):
        return True  # synthetically generated by immutables package
    if '$$' in name:
        return True  # synthetically generated CGLIB et al
    return False


def is_java_class(obj: type, name: str = '') -> bool:
...
def dependencies_satisfied(module: ModuleType, cls: type, done: Set[str]):
    bases = [infer_typename(b.class_) for b in cls.mro() if is_java_class(b)][1:]
    for base in bases:
        base_name = base.name
