...
def generate_stub_for_java_module(module_name: str, target: str) -> None:
    ...
    java_classes = {name.replace("$", "__"): cls for name, cls in module.__dict__.items()
                    if not is_python_internal(name) and is_non_internal_java_class(name, cls)}
...
def is_python_internal(name: str) -> bool:
    return name.startswith('__') and name.endswith('__')


def is_non_internal_java_class(name: str, obj: object) -> bool:
    if not inspect.isclass(obj) or not hasattr(obj, 'class_'):
        return False
    if obj.class_.isAnonymousClass() or obj.class_.isSynthetic():
        return False
    if re.match(".*\\$\\d+", name):
        return False  # anonymous inner classes
    if '$SerialForm' in name:
        return False  # synthetically generated by immutables package
    return True
...
def type_to_typestr(typ: type) -> TypeStr:
    if typ.__module__ == 'jpype._jclass':
        return TypeStr(typ.__name__.replace("$", "__"))
